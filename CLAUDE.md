...
6. **Ingest the completed exchange** — prompt and response together, only now, after the outcome is resolved: update every page the exchange touched (public and hidden), append world-dated entries to `log.md` for durable events, create pages and index lines for new entities, and migrate facts from `hidden/` to public pages once the hidden boundary (below) allows. Before any of that lands on a public page, index line, or log entry, trace it back to the actual words just narrated or the exchange archived in the session file — not to what's true on a hidden page, not to whatever makes the summary read well. If a clause you're about to write can't be pointed at a specific line the player just saw, it isn't public knowledge yet; cut it or route it to `hidden/` instead.
